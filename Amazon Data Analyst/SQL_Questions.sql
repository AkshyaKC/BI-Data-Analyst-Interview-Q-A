...
WITH Salary_CTE AS (
	SELECT dpt.department_id,
		...
		salary,
		ROW_NUMBER() OVER(
			PARTITION BY dpt.department
			ORDER BY et.salary DESC
		) AS RowNumber
	FROM employee_table AS et
		JOIN department AS dpt ON et.dept_id = dpt.department_id
)
SELECT department,
	full_name,
	salary
